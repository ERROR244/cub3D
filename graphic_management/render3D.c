...
/*   Updated: 2024/10/30 12:30:12 by khalil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int get_texture_line(t_window *window, t_render render, int wall_height, int current_y)
{
    unsigned int color;
    double texture_y;
    int texture_x;
    ...
    texture_height = (int)((double)max_height / (window->dist / window->tile_size));
...
int draw_rect(t_window *window, t_render render)
{
    ...
    while (++i < render.width && ret == 0)
    {
        ...
        while (++j < render.height && ret == 0)
            ret = my_mlx_pixel_put(window, render.x + i, render.y + j, get_texture_line(window, render, render.height, j));
    }
    return ret;
}
...
int render3d(t_window *window, int ret, int i)
{
    ...
    {
        window->dist = window->ray[i].distance * cos(window->ray[i].ray_a - window->pa);
        window->dist = fmax(window->dist, 0.1);
        window->disp = (window->window_width / 2) / tan(FOV_ANGLE / 2);
        window->wl3dh = (int)(window->tile_size / (window->dist / window->tile_size) * window->disp);

        if (window->wl3dh > window->window_hight)
            window->wl3dh = window->window_hight;

        if (window->ray[i].washitver)
            render.wall_hit_x = fmod(window->ray[i].ray_hit_y, window->tile_size) / window->tile_size;
        else
            render.wall_hit_x = fmod(window->ray[i].ray_hit_x, window->tile_size) / window->tile_size;
